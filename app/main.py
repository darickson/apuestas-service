...
def livez():
    ...
    return {"status": "alive", "uptime": _time.time() - _START_TIME}


@app.get("/readyz")
def readyz():
    """Readiness: verifica la conexión a PostgreSQL. 503 si no está lista."""
    try:
        ...
        raise HTTPException(status_code=503, detail={"status": "not-ready", "db": "down", "error": str(exc)})
